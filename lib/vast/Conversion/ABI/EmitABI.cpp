...
namespace vast {
    ...
    auto collect_abi_info(RootOp root_op, const DL &dl) -> abi_info_map_t< R > {
    ...
    namespace {
        ...
        struct abi_info_utils
        {
            ...
            auto get_inserter(auto &into) {
                return gap::overloaded{ [&](auto vals) {
                                           into.insert(into.end(), vals.begin(), vals.end());
                                       },
            ...
            void
            zip_ret(const auto &abi, const auto &results, const auto &concretes, auto &&yield) {
            ...
            , abi_info_utils< abi_transform< op_t > >
        {
            ...
            auto mk_direct(
                auto &bld, auto loc, const abi::direct &abi_arg, const mapped_arg_t &entry
            ) {
                auto &[original_type, args] = entry;

                auto opaque =
                    rewriter.template create< abi::DirectOp >(op.getLoc(), original_type, args);
            ...
            auto fold_arg(
                auto &bld, auto loc, const abi::indirect &abi_arg, const mapped_arg_t &entry
            ) -> mlir_value {
                auto &[original_type, args] = entry;
                return bld.template create< abi::IndirectOp >(op.getLoc(), original_type, args);
            }
            ...
            void mk_prologue(abi::FuncOp func) {
                ...
                auto entry = rewriter.createBlock(
                    &func.getBody(), {}, func.getFunctionType().getInputs(), arg_locs
                );
                ...
                auto abi_prologue = rewriter.template create< abi::PrologueOp >(
                    op.getLoc(), op.getFunctionType().getInputs(), fold_all_args
                );
            ...
            void get_body(abi::FuncOp func, const auto &arg_mapping) {
                rewriter.cloneRegionBefore(op.getBody(), func.getBody(), func.getBody().end());
        ...
        template< typename op_t >
        struct call_wrapper
            : abi_info_utils< call_wrapper< op_t > >
            , match_and_rewrite_state_capture< op_t >
        {
            using state_t = match_and_rewrite_state_capture< op_t >;
            ...
            auto get_callee() -> mlir::FunctionOpInterface {
                ...
                auto callee =
                    mlir::dyn_cast< mlir::FunctionOpInterface >(caller.resolveCallable());
                VAST_ASSERT(callee);
                return callee;
            }
            ...
            auto mk_direct(auto &bld, auto loc, const abi::direct &arg, values_t concrete_args)
                -> values_t {
                auto vals =
                    rewriter
                        .template create< Impl >(op.getLoc(), arg.target_types, concrete_args)
                        .getResults();
                return { vals.begin(), vals.end() };
            }

            auto mk_ret(
                auto &bld, auto loc, const abi::direct &arg, mlir::Value result,
                values_t concrete_args
            ) -> values_t {
                auto vals =
                    rewriter
                        .template create< abi::DirectOp >(loc, result.getType(), concrete_args)
                        .getResults();
                return { vals.begin(), vals.end() };
            }

            auto mk_ret(
                auto &, auto loc, const abi::ignore &, mlir::Value, values_t concrete_args
            ) -> values_t {
            ...
            auto mk_arg(auto &bld, auto loc, const abi::indirect &arg, mlir::Value concrete_arg)
                ...
                auto vals = bld.template create< abi::IndirectOp >(
                                   op.getLoc(), arg.target_types, concrete_arg
                )
                                .getResult();
                return vals;
            }
                ...
                return [&](auto &bld, auto loc) {
                    auto args = [&]() -> values_t {
                        ...
                        auto cooked_args = bld.template create< abi::CallArgsOp >(
                                                  loc, this->abified_args(), args_maker()
                        )
                                               .getResults();
                    ...
                    auto call = bld.template create< abi::CallOp >(
                        loc, op.getCallee(), this->abified_rets(), args
                    );
                    ...
                    auto to_yield = [&]() -> mlir::ResultRange {
                        ...
                        return bld
                            .template create< abi::CallRetsOp >(
                                loc, op.getResults().getType(), rets_maker(call.getResults())
                            )
                            .getResults();
                    }();

                    bld.template create< abi::YieldOp >(
                        loc, op.getResults().getType(), to_yield
                    );
                };
            }
            ...
            auto args_maker() {
                return [&](auto &bld, auto loc) {
                    ...
                    bld.template create< abi::YieldOp >(loc, this->abified_args(), out);
                };
            }
            ...
            auto rets_maker(mlir::ValueRange vals) {
                return [=, this](auto &bld, auto loc) {
                    ...
                    bld.template create< abi::YieldOp >(loc, op.getResultTypes(), out);
                };
            }

            auto make() {
                return rewriter.template create< abi::CallExecutionOp >(
                    op.getLoc(), op.getCallee(), op.getResults().getType(), op.getArgOperands(),
            ...
            , match_and_rewrite_state_capture< op_t >
        {
            ...
            template< typename Impl >
            auto mk_direct(
                auto &bld, auto loc, const abi::direct &arg, mlir::Value concrete_arg
            ) -> values_t {
                auto vals =
                    rewriter
                        .template create< Impl >(op.getLoc(), arg.target_types, concrete_arg)
                        .getResults();
                return { vals.begin(), vals.end() };
            }
            ...
            auto wrap_return(mlir::ValueRange vals) {
                return [=, this](auto &bld, auto loc) {
                    ...
                    bld.template create< abi::YieldOp >(loc, this->abified_rets(), out);
                };
            }

            hl::ReturnOp make() {
                auto wrapped = rewriter.template create< abi::EpilogueOp >(
                    op.getLoc(), this->abified_rets(), wrap_return(op.getResult())
                );

                return rewriter.template create< hl::ReturnOp >(
                    op.getLoc(), wrapped.getResults()
                );
            }
        };
        ...
        struct func_type : mlir::OpConversionPattern< Op >
        {
            ...
            func_type(TypeConverter &tc, const func_abi_info_t &abi_info_map, mcontext_t &mctx)
                : Base(tc, &mctx), tc(tc), abi_info_map(abi_info_map) {}

            mlir::LogicalResult matchAndRewrite(
                Op op, typename Op::Adaptor ops, conversion_rewriter &rewriter
            ) const override {
        ...
        struct call_op : mlir::OpConversionPattern< hl::CallOp >
        {
            ...
            call_op(TypeConverter &tc, const func_abi_info_t &abi_info_map, mcontext_t &mctx)
                : Base(tc, &mctx), tc(tc), abi_info_map(abi_info_map) {}
        ...
        struct return_op : mlir::OpConversionPattern< hl::ReturnOp >
        {
            ...
            return_op(TypeConverter &tc, const func_abi_info_t &abi_info_map, mcontext_t &mctx)
                : Base(tc, &mctx), tc(tc), abi_info_map(abi_info_map) {}
    ...
    struct EmitABI : EmitABIBase< EmitABI >
    {
        ...
        void runOnOperation() override {
            ...
            auto abi_info_map =
                collect_abi_info< mlir::FunctionOpInterface >(op, dl_analysis.getAtOrAbove(op));
