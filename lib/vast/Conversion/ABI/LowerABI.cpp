...
namespace vast {
        ...
        namespace {
            ...
            struct deconstructs_types
            {
                ...
                auto deconstruct_record(hl::RecordType record_type, abi::DirectOp direct) {
                    ...
                    return conv::abi::deconstruct_aggregate(
                        pattern, direct, as_lvalue, state.rewriter
                    );
                }
            ...
            struct reconstructs_types
            {
                ...
                convert_primitive_type(mlir_type target_type, abi::DirectOp direct) {
                    ...
                    auto stripped_lvalues = [&]() {
                        ...
                        for (auto v : direct.getOperands()) {
                            ...
                            auto cast = state.rewriter.template create< hl::ImplicitCastOp >(
                                direct.getLoc(), lvalue_type.getElementType(), v,
                    ...
                    return state.rewriter.template create< ll::Concat >(
                        direct.getLoc(), target_type, stripped_lvalues
                    );
                }
    ...
    struct LowerABI : ModuleConversionPassMixin< LowerABI, LowerABIBase >
    {
        ...
        void runOnOperation() override {
            auto &ctx   = getContext();
            auto config = config_t{ rewrite_pattern_set(&ctx), create_conversion_target(ctx) };
