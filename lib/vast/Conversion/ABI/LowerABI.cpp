...
namespace vast {
        ...
        namespace {
            ...
            struct abi_pattern_base : operation_conversion_pattern< Op >
            {
                ...
                abi_pattern_base(const mlir::DataLayout &dl, Args &&...args)
                    : base(std::forward< Args >(args)...), dl(dl) {}
                ...
                virtual values
                match_on(abi::IndirectOp indirect, state_capture state) const = 0;
            ...
            struct function_border_base : abi_pattern_base< Op >
            {
                ...
                logical_result rewrite(state_capture state) const {
                    ...
                    VAST_PATTERN_CHECK(
                        state.op.getNumResults() == to_replace.size(),
            ...
            struct deconstructs_types
            {
                ...
                values handle(abi::DirectOp direct) & {
                    ...
                    for (auto res_type : direct.getOperands().getTypes()) {
                        ...
                        init_values.insert(
                            init_values.end(), converted.begin(), converted.end()
                        );
                    }

                    VAST_CHECK(
                        init_values.size() == direct.getNumResults(), "{0} != {1}",
                        init_values.size(), direct.getNumResults()
                    );
                ...
                auto deconstruct_record(hl::RecordType record_type, abi::DirectOp direct) {
                    ...
                    auto as_ptr = [&]() -> mlir::Operation * {
                        ...
                        auto type = hl::PointerType::get(mctx, direct.getOperand(0).getType());
                        auto memory =
                            state.rewriter.template create< ll::Alloca >(direct.getLoc(), type);
                    ...
                    return conv::abi::deconstruct_aggregate(
                        pattern, direct, as_ptr, state.rewriter
                    );
                }

                mlir::Value
                convert_primitive_type(mlir_type target_type, abi::DirectOp direct) {
                    VAST_CHECK(
                        direct.getNumOperands() >= 1,
                    ...
                    VAST_CHECK(
                        can_concat_as(pattern.dl, direct.getOperands().getTypes(), target_type),
                        "Cannot do concat when converting {0}", direct
                    );

                    auto result_type = direct->getResult(0).getType();

                    return state.rewriter.template create< ll::Concat >(
                        direct.getLoc(), result_type, direct.getOperands()
                    );
                }
            ...
            struct reconstructs_types
            {
                ...
                mlir::Value
                reconstruct_record(hl::RecordType record_type, abi::DirectOp direct) {
                    return conv::abi::reconstruct_aggregate(
                        pattern, direct, record_type, state.rewriter
                    );
                }
                ...
                convert_primitive_type(mlir_type target_type, abi::DirectOp direct) {
                    ...
                    return state.rewriter.template create< ll::Concat >(
                        direct.getLoc(), target_type, direct.getOperands()
                    );
                }
                ...
                values handle(abi::DirectOp direct) & {
                    ...
                    co_yield state.rewriter.template create< ll::InitializeVar >(
                        direct.getLoc(), var.getResult().getType(), var, convert_values()
                    );
                }
            };
            ...
            struct prologue : function_border_base< abi::PrologueOp >
            {
                ...
                values match_on(abi::IndirectOp indirect, state_capture state) const override {
                    co_yield state.rewriter.template create< ll::Load >(
                        indirect.getLoc(), indirect.getResult().getType(), indirect.getValue()
                    );
                }
            };
            ...
            struct call_args : call_border< abi::CallArgsOp >
            {
                ...
                values match_on(abi::IndirectOp indirect, state_capture state) const override {
                    ...
                    auto var =
                        state.rewriter.template create< ll::Alloca >(indirect.getLoc(), type);
            ...
            struct call : operation_conversion_pattern< abi::CallOp >
            {
                ...
                logical_result matchAndRewrite(
                    ...
                    auto new_op = rewriter.create< hl::CallOp >(
                        op.getLoc(), op.getCallee(), op.getResults().getTypes(),
                        ops.getOperands()
                    );
    ...
    struct LowerABI : ConversionPassMixin< LowerABI, LowerABIBase >
    {
        ...
        void runOnOperation() override {
            auto &ctx                  = getContext();
            base_conversion_config cfg = { rewrite_pattern_set(&ctx),
