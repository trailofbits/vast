...
namespace vast::hl
{
    struct ExportFnInfo : ExportFnInfoBase< ExportFnInfo >
    {
        void runOnOperation() override
        {
            ...
            for (auto &op : *module.getBody())
            {
                ...
                for (auto &arg_type : fn.getArgumentTypes())
                    args.push_back(parse_typeinfo(arg_type));

                llvm::json::Array rets;
                for (auto &ret_type : fn.getResultTypes())
                    rets.push_back(parse_typeinfo(ret_type));
        ...
        llvm::json::Object parse_typeinfo(mlir::Type hl_type)
        {
            auto ptr_type = [&](auto ptr_type)
            {
                llvm::json::Object out;
                out["type"] = "ptr";
                out["element_type"] = parse_typeinfo(ptr_type.getElementType());
                return out;
            };

            auto lvalue_type = [&](auto lvalue_ty)
            {
                return parse_typeinfo(lvalue_ty.getElementType());
            };
