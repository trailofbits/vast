...
namespace vast::hl
{
    struct ExportFnInfo : ExportFnInfoBase< ExportFnInfo > {
        void runOnOperation() override {
            ...
            util::functions(mod, [&] (mlir::FuncOp fn) {
        ...
        struct Entry {
            ...
            self_t &type(const std::string &name) {
                raw["type"] = name;
                return *this;
            }

            self_t &type() {
                raw["type"] = t.getMnemonic();
                return *this;
            }
        ...
        llvm::json::Object parse_typeinfo(const mlir::DataLayout &dl, mlir::Type hl_type) {
            ...
            auto lvalue_type
                = [&](auto lvalue_ty) { return parse_typeinfo(dl, lvalue_ty.getElementType()); };

            auto void_type = [&](auto void_ty) { return Entry(void_ty).type().size(0u).take(); };

            auto scalars
                = [&](auto scalar) { return Entry(scalar).size(dl).type().modifs().take(); };
