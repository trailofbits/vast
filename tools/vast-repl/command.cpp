...
namespace vast::repl {
    namespace cmd {
        ...
        maybe_memory_buffer get_source_buffer(const state_t &state) {
            ...
            maybe_memory_buffer file_buffer =
                llvm::MemoryBuffer::getFile(state.source->c_str());
        ...
        // exit command
        //
        void exit::run(state_t &state) const { state.exit = true; }

        //
        // help command
        //
        void help::run(state_t &) const { throw_error("Not implemented!"); };
        ...
        void show::run(state_t &state) const {
            ...
            switch (what) {
                case show_kind::source:
                    return show_source(state);
                case show_kind::ast:
                    return show_ast(state);
                case show_kind::module:
                    return show_module(state);
                case show_kind::symbols:
                    return show_symbols(state);
                case show_kind::pipelines:
                    return show_pipelines(state);
                case show_kind::link:
                    return show_link(state, name);
            }
        };
        ...
        void meta::run(state_t &state) const {
            ...
            switch (action) {
                case meta_action::add:
                    add(state);
                    break;
                case meta_action::get:
                    get(state);
                    break;
            }
        }
