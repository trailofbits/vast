...
void f() {
    // CHECK: [[V:%[0-9]+]] = hl.var "v" : !hl.lvalue<!hl.named_type<<"s">>>
    struct s v;
    // CHECK: hl.var "x" : !hl.lvalue<!hl.int>
    // CHECK:   [[V1:%[0-9]+]] = hl.decl.ref [[V]] : !hl.lvalue<!hl.named_type<<"s">>>
    // CHECK:   [[V2:%[0-9]+]] = hl.member [[V1]] at "a" : !hl.lvalue<!hl.named_type<<"s">>> -> !hl.lvalue<!hl.int>
    // CHECK:   [[V3:%[0-9]+]] = hl.implicit_cast [[V2]] LValueToRValue : !hl.lvalue<!hl.int> -> !hl.int
    // CHECK:   hl.value.yield [[V3]] : !hl.int
    int x = v.a;
    // CHECK: [[V1:%[0-9]+]] = hl.decl.ref [[V]] : !hl.lvalue<!hl.named_type<<"s">>>
    // CHECK: [[V2:%[0-9]+]] = hl.member [[V1]] at "b" : !hl.lvalue<!hl.named_type<<"s">>> -> !hl.lvalue<!hl.short>
