...
int int_ptr_cmp(unsigned long a, void* b) {
    // CHECK: hl.cmp eq [[A:%[0-9]+]], [[B:%[0-9]+]] : !hl.long<unsigned>, !hl.named_type<<"uintptr_t">> -> !hl.int
    return a == (uintptr_t)b;
}

int ptr_int_cmp(void *a, unsigned b) {
    // CHECK: hl.cmp eq [[A:%[0-9]+]], [[B:%[0-9]+]] : !hl.named_type<<"uintptr_t">>, !hl.long<unsigned> -> !hl.int
