...
// CHECK:  hl.field "state" : !hl.elaborated<!hl.record<"State">>
struct Element {
    int z;
    enum State { SOLID, LIQUID, GAS, PLASMA } state;

// CHECK: hl.var "oxygen" : !hl.lvalue<!hl.elaborated<!hl.record<"Element">>>
// CHECK:  [[V1:%[0-9]+]] = hl.const #hl.integer<8> : !hl.int
// CHECK:  [[V2:%[0-9]+]] = hl.enumref "GAS" : !hl.int
// CHECK:  [[V3:%[0-9]+]] = hl.implicit_cast [[V2]] IntegralCast : !hl.int -> !hl.elaborated<!hl.record<"State">>
// CHECK:  hl.initlist [[V1]], [[V3]] : (!hl.int, !hl.elaborated<!hl.record<"State">>) -> !hl.elaborated<!hl.record<"Element">>
} oxygen = { 8, GAS };

void foo(void) {
    // CHECK: hl.var "e" : !hl.lvalue<!hl.elaborated<!hl.record<"State">>>
