// RUN: %vast-cc --from-source %s | %vast-opt --vast-hl-lower-types | %file-check %s

// CHECK-LABEL: hl.func @main () -> si32
int main()
{
    // CHECK: hl.var "x" : !hl.lvalue<ui1> =  {
    // CHECK:   [[V1:%[0-9]+]] = hl.const #hl.bool<false> : ui1
    // CHECK:   hl.value.yield [[V1]] : ui1
    // CHECK: }
    bool x = false;

    // CHECK: hl.var "y" : !hl.lvalue<si8> =  {
    // CHECK:   [[V2:%[0-9]+]] = hl.const #hl.integer<72> : si8
    ...
    // CHECK:   [[V4:%[0-9]+]] = hl.const #hl.integer<1> : si32
    // CHECK:   [[V5:%[0-9]+]] = hl.const #hl.integer<2> : si32
    ...
    // CHECK:   [[V7:%[0-9]+]] = hl.const #hl.integer<142> : si32
    ...
    // CHECK:   [[V9:%[0-9]+]] = hl.const #hl.float<91.019999999999996> : f64
    ...
    // CHECK:   [[V11:%[0-9]+]] = hl.const #hl.float<0.000000e+00> : f64
    // CHECK:   [[V12:%[0-9]+]] = hl.implicit_cast [[V11]] FloatingCast : f64 -> f32
    // CHECK:   [[V13:%[0-9]+]] = hl.const #hl.float<5.000000e-01> : f64
    // CHECK:   [[V14:%[0-9]+]] = hl.implicit_cast [[V13]] FloatingCast : f64 -> f32
    // CHECK:   [[V15:%[0-9]+]] = hl.const #hl.float<1.000000e+00> : f64
